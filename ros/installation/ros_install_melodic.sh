...
case $version in
  ...
  ;;
  *)
    echo ">>> {ERROR: This script will only work on Ubuntu Bionic Beaver (18.04).}"
    exit 0
esac
echo ""
echo ">>> {Ubuntu Bionic Beaver 18.04 is fully compatible with Ubuntu Bionic Beaver 18.04}"
echo ""
echo "---------------------------------------------------------------------------------------------------------"
...
#Installing curl: Curl instead of the apt-key command, 
# which can be helpful if you are behind a proxy server: 
